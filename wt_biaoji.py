...
import func.funcs_biaoji as F
import func.vb_biaoji as vb
curpath = os.path.abspath(os.path.dirname(__file__))
sys.path.append(curpath)

##################### process wannier tools ###########################
class wt():
    def __init__(self, filename, index_biaoji, color_charity):
        ...
        self.index_biaoji = index_biaoji
        self.realNodes = np.dot(F.get_nodes_biaoji(self.file_path, filename, index_biaoji), self.rotMat)
        self.arc_origionalP, self.arc_vector = F.read_arc_range(self.file_path, "WT.out")
        self.charity_biaoji = F.charity_biaoji(self.file_path, "wanniercenter3D_Weyl.dat", index_biaoji)
    ...
    def visuzl_BZ(self):
        ...
        plt = vb.visualize_BZ_matplotlib(points, ridges, facets, reciprocal_lattice_vectors, self.file_path, self.filename, self.index_biaoji, self.color_charity)
        return plt
    ...
    def plot_arc(self, cm, filename="arc.dat_r", figname="arc_r.png"):
        ...
        if osp.exists(osp.join(self.file_path, "wanniercenter3D_Weyl.dat")):
            ...
            charity = self.charity_biaoji
            for ii in range(len(charity)):
                if charity[ii] >= 1:
                    ax.text(nodes2d[ii, 0], nodes2d[ii, 1], str(self.index_biaoji[ii]), color=self.color_charity[0], fontsize=15, fontname="Times New Roman")
                    ax.scatter(nodes2d[ii, 0], nodes2d[ii, 1], s=self.pointsize, color=self.color_charity[0], marker="o")
                elif charity[ii] <= -1:
                    ax.text(nodes2d[ii, 0], nodes2d[ii, 1], str(self.index_biaoji[ii]), color=self.color_charity[1], fontsize=15, fontname="Times New Roman")
...
def manipulate():
    ...
    print("-------------- 输入的标记 index ----------------")
    index_biaoji = np.array([int(ii) for ii in sys.argv[1:]])
    print(index_biaoji)
    ...
    wtt = wt(filename_nodes, index_biaoji, color_charity)
    wtt.stru_info()
    wtt.write2D(filename_nodes) 
    wtt.plot_arc(cm, filename_arc_r, "arc_nodes_r_biaoji.png")
    wtt.plot_arc(cm, filename_arc_l, "arc_nodes_l_biaoji.png")
    plt = wtt.visuzl_BZ()
    #plt.show()
