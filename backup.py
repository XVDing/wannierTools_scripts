...
import func.visualize_BZ_nodes_charity as vb
...
class wt():
    ...
    def visuzl_BZ(self):
        ...
        vb.visualize_BZ_matplotlib(points, ridges, facets, reciprocal_lattice_vectors, self.file_path)
    ...
    def plot_arc(self, filename="arc.dat_r", figname="arc_r.png", color_nodes="blue"):
        ...
        if osp.exists(osp.join(self.file_path, "wanniercenter3D_Weyl.dat")):
            ...
            for ii in range(len(charity)):
                if charity[ii] >= 1:
                    #ax.text(nodes2d[ii, 0], nodes2d[ii, 1], str(charity[ii]), color="red", fontsize=10, fontname="Times New Roman")
                    ax.scatter(nodes2d[ii, 0], nodes2d[ii, 1], s=15, color="red", marker="o")
                elif charity[ii] <= -1:
                    #ax.text(nodes2d[ii, 0], nodes2d[ii, 1], str(charity[ii]), color="green", fontsize=10, fontname="Times New Roman")
                    ax.scatter(nodes2d[ii, 0], nodes2d[ii, 1], s=15, color="cyan", marker="o")
        else:
            ax.scatter(nodes2d[:, 0], nodes2d[:, 1], s=20, marker="o", color="red")
        ########################################################################
...
def manipulate():
    ...
    wtt = wt(filename_nodes)
    wtt.stru_info()
    wtt.write2D(filename_nodes) 
    wtt.plot_arc(filename_arc_r, "arc_nodes_r.png", color_nodes="blue")
    wtt.plot_arc(filename_arc_l, "arc_nodes_l.png", color_nodes="blue")
