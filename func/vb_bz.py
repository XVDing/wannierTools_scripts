...
def visualize_BZ_matplotlib(points,ridges,facets,reciprocal_lattice_vectors, filepath, filename, color_charity):
    import matplotlib as mpl
    ...
    if osp.exists(osp.join(filepath, "wanniercenter3D_Weyl.dat")):
        charity = charity_data(filepath, "wanniercenter3D_Weyl.dat")
        for jj in range(len(charity)):
            if charity[jj] <= -1:
                ax.text(k1[jj], k2[jj], k3[jj], str(jj), color=color_charity[1], fontsize=11, fontname="Times New Roman")
                ax.scatter(k1[jj], k2[jj], k3[jj], marker='o', color=color_charity[1],s=15,alpha=0.8)
            elif charity[jj] >= 1:
                ax.text(k1[jj], k2[jj], k3[jj], str(jj), color=color_charity[0],  fontsize=11, fontname="Times New Roman")
                ax.scatter(k1[jj], k2[jj], k3[jj], marker='o', color=color_charity[0],s=15,alpha=0.8)
    else:
        for ii in range(len(k1)):
            ax.scatter(k1[ii], k2[ii], k3[ii], c=color_charity[0], marker='o',s=15,alpha=0.8)
            ax.text(k1[ii], k2[ii], k3[ii], str(ii), color=color_charity[0],  fontsize=11, fontname="Times New Roman")
    ax.set_axis_off()
    ax.view_init(elev=16, azim=-70)
    plt.savefig('bz.png', dpi=300, bbox_inches='tight')
    ax.view_init(elev=0, azim=90)
    plt.savefig('bz_side.png', dpi=300, bbox_inches='tight')
    ax.view_init(elev=45, azim=-45)
    plt.savefig('bz_top.png', dpi=300, bbox_inches='tight')
    return plt
...
if __name__ == "__main__":
   ...
   filepath = os.getcwd()  
   print(filepath) 
   reciprocal_lattice_vectors=read_poscar(filepath, 'POSCAR')   
   ...
   visualize_BZ_matplotlib(points, ridges, facets, reciprocal_lattice_vectors, filepath, filename, color_charity)
