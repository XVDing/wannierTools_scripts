...
def get_NK(filepath, filename):
    file = osp.join(filepath, filename)
    data = open(file, 'r').readlines()
    i = 0
    for item in data:
        if 'Nk' in item:
            Nk1 = int(item.split(":")[1])
            Nk2 = int(data[i+1].split(":")[1])
            Nk3 = int(data[i+2].split(":")[1])
            break
        i = i + 1
    Nk = [Nk1, Nk2, Nk3]
    return Nk

def get_ticks(filepath, filename):
    ...
    for i in range(len(data_lines)):
        if "yrange" in data_lines[i]:
            if ")" not in data_lines[i]:
                ...
                ticks_label = [ticks[i].split()[0].split('\"')[1] for i in range(len(ticks))]
                ticks_data = [np.around(float(ticks[i].split()[-1]), 10) for i in range(len(ticks))]
                print(ticks_label)
                print(ticks_data)
                break
            else:
...
def readNk1_WtOUT(filepath, filename):
    file = osp.join(filepath, filename)
    data = open(file, 'r').readlines()
    for f in data:
        if "Nk1" in f:
