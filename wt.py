...
class wt():
    ...
    def matrix_data(self):
        ...
        pos2D = np.around(np.array([rot_pos[0, 0:2], rot_pos[1, 0:2]]), 10)
        ...
        return np.around(rot_pos, 10), np.around(rep_surface, 10), pos2D, rep_2D, n
    ...
    def write_node2d_ssband(self, nodes1):
        ...
        with open(osp.join(self.file_path, "node2d_SSband.dat"), 'w') as nd1:
            ...
            for item in nodes:
                for i in range(2):
                    nd1.write(str(np.round(item[i], 10)).ljust(14, "0") + "   ")
                nd1.write("\n")
        nd1.close()
    ...
    def write2D(self, filename):
        ...
        with open(osp.join(self.file_path, "nodes2D_arc.dat"), 'w') as arcnode:
            ...
            for item1 in nodes2D:
                for i in range(2):
                    arcnode.write(str(np.round(item1[i], 10)).ljust(14, "0") + "   ")
                arcnode.write("\n")
        arcnode.close()
        ########################## nodes for ssband ###########################
    ...
    def stru_info(self):
        ...
        print(" ")
        print(np.around(self.rot_pos, 10))
        print(" ")
        print("--- rotated rep surface -----")
        print(" ")
        print(np.around(self.rep_surface, 10))
        print(" ")
